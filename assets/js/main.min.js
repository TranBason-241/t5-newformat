$(document).ready(function () {
  ...
  // click to
  listClick = document.querySelectorAll(".buy-action p");
  listClick.forEach((element) => {
    element.onclick = () => {
      window.location.href = "https://www.gucci.com/us/en/ca/whats-new/adidas-x-gucci/adidas-x-gucci-ready-to-wear-c-adidas-gucci-ready-to-wear";
    };
  });
});

trickChangeWidthSlide = (swiperSlide) => {
  // Folowing z-index
  // stack 1 2 3 4 <=> 1 0 -1 -2
  var elementsWithTranslateAndPositiveX = swiperSlide.querySelectorAll('[style*="translate3d"]');
  var elementsWithPositiveX = [];

  elementsWithTranslateAndPositiveX.forEach(function (element) {
    var style = element.style;
    var translateValues = getTransformValue(style.transform, "translate3d");

    console.log(translateValues[0]);

    if (translateValues && translateValues[0] > 0 && style.zIndex == "-1") {
      elementsWithPositiveX.push(element);
      element.classList.add("l-03");
    }

    if (translateValues && translateValues[0] < 0 && style.zIndex == "-1") {
      console.log(element);
      elementsWithPositiveX.push(element);
      element.classList.add("r-03");
    }
  });

  // In ra các phần tử thỏa mãn điều kiện
  elementsWithPositiveX.forEach(function (element) {
    console.log(element);
  });
};

// Hàm hỗ trợ để lấy giá trị từ chuỗi transform
function getTransformValue(transform, propertyName) {
  var match = transform.match(new RegExp(propertyName + "\\(([-\\d]+)", "i"));
  if (match && match[1]) {
